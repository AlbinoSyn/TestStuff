...
async function handleCSSOutput(file, css) {
  ...
  const outfile = isTestStuffPackage(file) ?
    `${outputDir}/TestStuff.css` :
    `${outputDir}/style.css`;
...
function isTestStuffPackage(file) {
  return path.normalize(file).includes('packages/TestStuff/');
}


async function compileCSS() {
  ({ default: chalk } = await import('chalk'));
  const files = await glob('packages/{,@TestStuff/}*/src/style.scss');
